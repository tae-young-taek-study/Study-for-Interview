...
### 설명
Spring MVC 는 웹 애플리케이션을 위한 MVC 패턴 기반의 웹 프레임워크입니다. 데이터 관리 및 비즈니스 로직을 처리하는 Model, 화면을 구성하는 View, Model 과 View 를 중개하는 Controller 로 구성되어 있습니다.
### 정리
- Spring MVC: 웹 어플리케이션 개발을 위한 MVC 패턴 기반의 웹 프레임워크
- 구성 컴포넌트:
  - Dispatcher Servlet: 클라이언트의 요청을 먼저 받아들이는 서블릿, 요청에 맞는 컨트롤러에게 요청 전달
  - Handler Mapping: 해당 요청이 어떤 컨트롤러에게 온 요청인지 검사
  - Controller: 클리이언트의 요청을 받아 처리하여 Dispatcher Servlet 에 전달
  - ViewResolver: View 의 이름을 통해 알맞은 View 를 찾음
  - View: 사용자에게 보여질 화면 구성
- MVC
  - Model
    - 데이터 관리 및 비즈니스 로직 처리
    - DAO, DTO, service ...
  - View
    - 비즈니스 로직 처리 결과를 화면에 표현
    - json, html, jsp, thymeleaf, ...
  - Controller
    - 사용자의 요청 처리 및 Model 과 View 중개 처리.
### 참조
...
### 설명
클라이언트가 요청을 보내면 Dispatcher Servlet 은 Handler Mapping 을 통해 알맞은 Controller 를 찾고 Handler Adapter 를 통해 해당 Controller 에게 요청을 전달합니다. Controller 와 Model 을 통해 비즈니스 로직을 처리한 후 View 이름을 반환합니다. View Resolver 는 View 이름을 통해 View 를 찾고 Dispatcher Servlet 은 View 에 전달할 데이터를 추가하고 이를 반환하는 흐름입니다.
### 정리
![SpringMVC_Flow.png](SpringMVC_Flow.png)
